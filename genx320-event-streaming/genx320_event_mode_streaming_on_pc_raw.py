    ...
    try:
        with Camera(args.port, baudrate=args.baudrate, crc=args.crc, seq=args.seq,
            ...
            while True:
                ...
                shape = camera._channel_shape(camera.get_channel(name='events'))
                if not shape:
                    time.sleep(0.01)
                    continue
                ...
                # Each event: 32-bit word, aka 4 bytes
                if (len(data) % 4) != 0:
                    print(f"Warning: misaligned packet: {len(data)} bytes (not multiple of 4)")
                    continue

                # dtype is 32-bit unsigned integer
